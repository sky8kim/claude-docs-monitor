...
# ============================================================
# AI 요약 생성 (Gemini 우선 → 규칙 기반 fallback)
# ============================================================

def _rule_based_summary(changes: list) -> str:
    """규칙 기반 변경사항 요약 (API 없이 diff 분석)"""
    parts = []
    for c in changes:
        diff = c.get("diff", "")
        added = 0
        ...
        for line in diff.splitlines():
            if line.startswith("+") and not line.startswith("+++"):
                added += 1
                # ## 헤더 변경 감지
                header = re.match(r'^\+\s*(#{1,3})\s+(.+)', line)
                if header:
                    changed_sections.append(header.group(2).strip())
                # 키워드 감지 (new, deprecated, removed, added 등)
                for kw in ["new", "deprecated", "removed", "added", "updated", "changed", "breaking"]:
                    if kw in line.lower() and kw not in keywords:
                        keywords.append(kw)
            elif line.startswith("-") and not line.startswith("---"):
                removed += 1

        entry = f"📌 [{c['category']}] {c['name']}\n"
        entry += f"   URL: {c['url']}\n"
        entry += f"   변경량: +{added}줄 / -{removed}줄"
        if changed_sections:
            entry += f"\n   변경 섹션: {', '.join(changed_sections[:5])}"
        if keywords:
            entry += f"\n   키워드: {', '.join(keywords)}"
        parts.append(entry)
...
def _summarize_with_gemini(changes_text: str) -> str:
    """Gemini API로 변경사항 요약"""
    resp = requests.post(
        ...
        json={
            "contents": [{
                "parts": [{"text": f"""다음은 Claude 공식 문서의 변경사항입니다.
...
def summarize_changes(changes: list) -> str:
    """변경사항 요약 (1순위: Gemini → 2순위: 규칙 기반 fallback)"""
    changes_text = "\n\n---\n\n".join([
        f"문서: {c['name']}\nURL: {c['url']}\n카테고리: {c['category']}\nDiff:\n{c['diff'][:2000]}"
        for c in changes
    ])
    ...
    if GEMINI_API_KEY:
        try:
            result = _summarize_with_gemini(changes_text)
            print("✅ Gemini 요약 완료")
            return result
        except Exception as e:
    ...
    # 2순위: 규칙 기반 요약
    print("📋 규칙 기반 요약 사용")
    return _rule_based_summary(changes)
